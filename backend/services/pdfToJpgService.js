...
async function getPdfInfo(filePath) {
  ...
  try {
    ...
    if (err.message && (err.message.includes('encrypt') || err.name === 'EncryptedPDFError')) {
      const customErr = new Error('Unable to process this PDF. The file is password-protected. Please remove the password and try again.');
      customErr.statusCode = 400;
      throw customErr;
    }
    const customErr = new Error('Unable to process this PDF. The file may be corrupted or not a valid PDF.');
    customErr.statusCode = 400;
    throw customErr;
  }
}
...
async function convertPdfToJpg({
  ...
  try {
    ...
    const customErr = new Error('Unable to parse PDF document. The file may be corrupted.');
    customErr.statusCode = 400;
    throw customErr;
  }
  ...
  // Calculate resolution & scale factor (PDF default is 72 pt/in)
  const targetDpi = DPI_MAP[dpi] || 150;
  const scale = targetDpi / 72;
  const targetQuality = QUALITY_MAP[quality] || 80;

  const baseName = sanitizeFilename(path.parse(originalFilename).name);
  ...
  for (let i = 0; i < targetPages.length; i++) {
    ...
    const jpgBuffer = canvas.toBuffer('image/jpeg', targetQuality);
    ...
    convertedPages.push({
      ...
      fileSizeFormatted: formatBytes(jpgBuffer.length),
      downloadUrl: `/api/download/${sessionId}/${encodeURIComponent(filename)}`
    });
  }

  // Generate ZIP of all pages
  const zipFilename = `${baseName}_all_pages.zip`;
  const zipPath = path.join(sessionDir, zipFilename);
  await createZipArchive(sessionDir, convertedPages.map(p => p.filename), zipPath);
  ...
  return {
    ...
    zipDownloadUrl: `/api/download-zip/${sessionId}`
  };
}
 ...
 */
function createZipArchive(sessionDir, filenames, zipOutputPath) {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      if (archiverPkg.ZipArchive) {
        archive = new archiverPkg.ZipArchive({ zlib: { level: 6 } });
      } else if (typeof archiverPkg === 'function') {
        archive = archiverPkg('zip', { zlib: { level: 6 } });
      } else if (archiverPkg.default) {
        archive = typeof archiverPkg.default === 'function'
          ? archiverPkg.default('zip', { zlib: { level: 6 } })
          : new archiverPkg.default.ZipArchive({ zlib: { level: 6 } });
      } else {
    ...
    if (fin && typeof fin.catch === 'function') {
      fin.catch(reject);
    }
  });
}
